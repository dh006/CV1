...
const BestSellerPage = () => {
  ...
  const bestSellers = useMemo(() =>
    products.slice().sort((a, b) => (b.buyTurn || 0) - (a.buyTurn || 0)),
    [products]
  );
    ...
    <div style={s.container}>
      <div style={s.hero}>
        <div style={s.heroContent}>
          <nav style={s.breadcrumb}><Link to="/" style={s.breadLink}>Trang chủ</Link> / <strong>Bán Chạy</strong></nav>
          <h1 style={s.heroTitle}>HÀNG BÁN CHẠY</h1>
          <h2 style={s.heroSub}>BEST SELLERS</h2>
        </div>
        <div style={s.heroImgBox}>
          <img src="https://cdn.hstatic.net/files/1000360022/collection/hang-ban-chay_1cd58c4b42b9419088f85830d5d9527c.jpg" alt="Bán chạy" style={s.heroImg} />
        </div>
      </div>
      <div style={s.toolbar}>
        <p style={s.count}>{loading ? "Đang tải..." : `${bestSellers.length} sản phẩm`}</p>
      </div>
      <div style={s.grid}>
        {loading
